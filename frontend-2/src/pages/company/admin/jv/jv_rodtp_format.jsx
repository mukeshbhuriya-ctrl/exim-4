...
export function JvRodtpFormatContent() {
  ...
  useEffect(() => {
    ...
    const fetchFormat = async () => {
      ...
      try {
        const res = await fetch(`${BACKEND_URL}/api/company/admin/jv/jv-rodtp-format`, {
          method: 'GET',
          credentials: 'include',
        })
        ...
        if (!res.ok) {
          ...
          throw new Error(data?.detail || data?.message || `Failed to load JV RODTP format (${res.status})`)
        }
        ...
        message.error(err instanceof Error ? err.message : 'Failed to load JV RODTP format')
      } finally {
  ...
  const handleSaveFormat = async () => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`${BACKEND_URL}/api/company/admin/jv/create-jv-rodtp-format`, {
        method: 'POST',
        credentials: 'include',
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        throw new Error(data?.detail || data?.message || `Save JV RODTP format failed (${res.status})`)
      }
      setFormatMode('update')
      message.success(data?.message || 'JV RODTP format saved successfully')
    } catch (err) {
      message.error(err instanceof Error ? err.message : 'Failed to save JV RODTP format')
    } finally {
      setSubmitting(false)
    }
  }
